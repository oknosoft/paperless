...
- Пример взаимодействия с оборудованием на низком уровне. Функция `do_furn_cut()` модуля [scancmd](js/scancmd.js) отправляет строку с адресом, номером порта и координатами в релейную службу, реализованную на Node.js (файл [relay_http](js/relay_http.js)), которая перенаправляет сырые данные по winsocket в управляющий компьютер фурнитурной станции
